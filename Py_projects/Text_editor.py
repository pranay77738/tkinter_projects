...
def open_file():
    ...
    filepath=askopenfilename(filetypes=[("Text Files","*.txt"),("Text Files","*.*")])
    if not filepath:
        return
...
def save_file():
    ...
    filepath=asksaveasfilename(filetypes=[("Text Files","*.txt"),("Text Files","*.*")])
    if not filepath:
        return
...
btn_save = tk.Button(fr_button,text='Save',command=save_file)
